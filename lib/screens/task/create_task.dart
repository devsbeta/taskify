...
import 'package:taskify/bloc/status_multi/status_multi_bloc.dart';
...
class _CreateTaskState extends State<CreateTask> {
 ...
 String? role;
  ...
  Future<void> _getRole() async {
    role = await HiveStorage.getRole() ?? 'member'; // Default to 'member'
    setState(() {});
  }

  bool _isFieldEnabled(String fieldType) {
    if (role == null) return false; // Disable until role is fetched
    if (fieldType == 'status') {
      return role == 'member' || role == 'admin'; // Enable status for members/admins
    }
    return role == 'admin'; // Enable other fields only for admins
  }

  // New method to wrap UsersField with Visibility
  Widget _buildUsersField() {
    return Visibility(
      visible: role == 'admin', // Show only for admins
      child: UsersField(
        isCreate: widget.isCreate!,
        usersname: usersName ?? [],
        project: const [],
        usersid: widget.usersid!,
        enabled: _isFieldEnabled('users'), // Respect enabled state
  ...
  void initState() {
    super.initState();
    _getRole();
    context.read<StatusMultiBloc>().add(StatusMultiList());
    if (widget.isCreate == false) {
      String? formattedstart;
      String? formattedend;

      selectedusersNameId = widget.usersid;
      if (widget.start != null && widget.start!.isNotEmpty && widget.start != "") {
      ...
      descController = TextEditingController(text: removeHtmlTags(widget.desc!));
  ...
  Widget _form(tasks, isLightTheme, isPaginatedState) {
    return Column(
      ...
      children: [
        ...
        _buildUsersField(), // Replaced UsersField with _buildUsersField
        SizedBox(height: 15.h),
        StatusField(
          isRequired: true,
          status: widget.statusId != null && widget.statusId! > 0 ? widget.statusId : null,
          isCreate: widget.isCreate!,
          name: selectedStatus ?? widget.status ?? "",
        ...
        CustomTextField(
          ...
          enabled: _isFieldEnabled('description'),
          onSaved: (value) {},
          onFieldSubmitted: (value) {},
          isLightTheme: isLightTheme,
          isRequired: false,
        ...
        Padding(
          ...
          child: DatePickerWidget(
            ...
            onTap: _isFieldEnabled('date')
                ...
                : () {
                    flutterToastCustom(
                      msg: 'Only admins can change dates',
                      color: Colors.red,
                    );
                  },
            isLightTheme: isLightTheme,
        ...
        CreateCancelButtom(
          ...
          onpressCreate: widget.isCreate == true
              ? () async {
                  if (_isFieldEnabled('create')) {
                    print('tapped on create');
                    isPaginatedState == true ? _onCreate() : null;
                  } else {
                    flutterToastCustom(
                      msg: 'Only members and admins can create tasks',
                      color: Colors.red,
                    );
                  }
                }
              : () {
                  if (_isFieldEnabled('update')) {
                    isPaginatedState == true ? _onUpdateTask() : null;
                  } else {
                    flutterToastCustom(
                      msg: 'Only members and admins can update tasks',
                      color: Colors.red,
                    );
                  }
                },
